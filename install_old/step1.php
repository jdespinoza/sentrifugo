...
<form name="frmstep1" id="frmstep1" method="post" action="" class="frm_install">
    <h3 class="page_title">Pre Requisitos</h3>
...
?>
		<li>
			...
			<?php if($req_value == 0){?>
					<?php if($req == 'php' ){?>
				   			<a href="<?php echo PHPURL;?>" target="_blank" style="text-decoration: none;"><div class="error-txt" id = "phplink">La versión actual de php no cumple con el proceso de instalación.</div></a>
				    <?php } else if($req == 'pdo_mysql') {?>
				           <a href="<?php echo PDOURL;?>" target="_blank" style="text-decoration: none;"><div class="error-txt" id = "phplink">La extensión PDO-Mysql está deshabilitada en su archivo php.ini.</div></a>
				    <?php } else if($req == 'gd') {?>
		                   <a href="<?php echo GDURL;?>" target="_blank" style="text-decoration: none;"><div class="error-txt" id = "phplink">El módulo de la biblioteca GD está deshabilitado en su archivo php.ini.</div></a>
		             <?php } else if($req == 'openssl') {?>
		                   <a href="<?php echo OPENSSLURL;?>" target="_blank" style="text-decoration: none;"><div class="error-txt" id = "phplink">El módulo Open SSL está deshabilitado en su archivo php.ini.</div></a>
			<?php }}?>
		</li>
<?php
	}
...
?>
        ...
        <div class="folder_title">Permisos de archivos y carpetas</div>
         <ul class="progress">
<?php
...
{
?>
    <button name="next" id="next" type="button" onclick="window.location='index.php?s=<?php echo sapp_Global::_encrypt(2);?>';">Continuar</button>
<?php
}
