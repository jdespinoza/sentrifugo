        ...
        if($sizeOfWorksheet > 1)
        {
            ...
            try
            {
                ...
                for($i=2; $i <= $sizeOfWorksheet; $i++ )
                {
                    ...
                    if(empty($rowData[1]))
                    {
                        $err_msg = "Nombre no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(empty($rowData[2]))
                    {
                        $err_msg = "Apellido no puede estar vacio en la fila  ".$i.".";
                        break;
                    }
                    if(empty($rowData[3]))
                    {
                        $err_msg = "Id Empleado no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if (!empty($rowData[3]) && strlen($rowData[3]) > 5)
                    {
                        $err_msg = "El largo del id del empleado debe ser menor o igual a 5 en la fila ".$i.".";
                        break;
                    }
                    if(empty($rowData[4]))
                    {
                        $err_msg = "Rol no puede ser vacion en la fila ".$i.".";
                        break;
                    }
                    if(empty($rowData[5]))
                    {
                        $err_msg = "Correo no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(!in_array($rowData[4], $mng_roles_arr) && empty($rowData[7]))
                    {
                        $err_msg = "Departamento no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(empty($rowData[8]))
                    {
                        $err_msg = "Gerente de informes no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(!empty($rowData[10]) && empty($rowData[9]))
                    {
                        $err_msg = "Profesion no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(!empty($rowData[9]) && empty($rowData[10]))
                    {
                        $err_msg = "Posicion no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(empty($rowData[11]))
                    {
                        $err_msg = "Estado del empleado no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(empty($rowData[12]))
                    {
                        $err_msg = "Fecha de ingreso no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(in_array(strtolower($rowData[11]), $dol_emp_stat_arr) && empty($rowData[13]))
                    {
                        $err_msg = "Fecha de paretida no puede estar vacio en la fila ".$i.".";
                        break;
                    }
                    if(!in_array($rowData[11], $dol_emp_stat_arr) && !empty($rowData[13]) && in_array($emp_stat_arr[strtolower($rowData[11])],$emp_stat_arr))
                    {
                        $err_msg = "La fecha de salida debe de estar vacia para '".$rowData[11]."' en la fila ".$i.".";
                        break;
                    }
